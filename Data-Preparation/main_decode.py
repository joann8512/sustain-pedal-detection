...
def load_save_pedal_onset_npy(track_id):
    ...
    """
    audio_path = os.path.join(DF_ONSET.loc[[track_id]].filepath.values[0])  # DIR_PEDAL_ONSET,
    src, _ = librosa.load(audio_path, sr=SR)
    src = src.astype(np.float16)
    #npy_path = os.path.join(DIR_PEDAL_ONSET_NPY,DF_ONSET.loc[[track_id]].filepath.values[-1].split('.')[0]+'.npy')
    npy_path = os.path.join(DIR_PEDAL_ONSET_NPY, 'test', audio_path.split('/')[-2], audio_path.split('/')[-1].replace('.wav', '.npy'))
    np.save(npy_path, src)
    
    #print('  {}'.format(DF_ONSET.loc[[track_id]].filename.values[0]))
    print('Saving at {}'.format(npy_path))
...
def decode_pedal_onset():
    ...
    """
    tmp_fold = ['test']
    for folder in tmp_fold:  # FOLDERS
        print('{}..'.format(folder))
